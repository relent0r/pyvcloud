...
The utils module has an added function to extract and compare a UUID from an href.
The client module has an added class to help set the object href based on moRef and the client class has an added function get_uriobject_uuid which will build the href from the uuid and object type.

I modifed this for use as a service provider where multitenancy stops the use of names as inputs and data structures require the use of moRefs in the source CMDB to reference vCloud objects.
